...
This checkout contains the next major API revision. Rate requests are preferences, not guarantees. Native display information, Flutter frame cadence, and display-link/browser callback cadence are separate measurements. Physical presentation FPS is unavailable unless a qualified presentation source is added.
...
See [migration notes](docs/migration.md), [implementation status](docs/implementation-status.md), and the [audit checklist](docs/improvement-plan.md).
